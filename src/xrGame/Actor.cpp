...
void CActor::HitMarkMonster(Fvector dir, float damage)
{
	...
	auto s = CurrentGameUI()->AddCustomStatic("monster_claws", false);
...
float CActor::currentFOV()
{
	...
	)
	{
		if (pWeapon->GetZoomFactor() == 0)
			return atan(tan(g_fov * (0.5 * PI / 180)) / 1.25) / (0.5 * PI / 180);
		else
...
void CActor::OnItemDrop(CInventoryItem *inventory_item, bool just_before_destroy)
{
	...
	// Pavel: ïðè ïðîäàæå â ÌÏ ãðàíàòà óäàëÿåòñÿ ó èãðîêà
	// È ïîñëå ýòîãî íåëüçÿ äîñòàòü íîâóþ
	// Ïîýòîìó çàêîììåíòèðîâàë ïðîâåðêó íà just_before_destroy
