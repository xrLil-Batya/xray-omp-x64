...
bool script_r_eof(IReader *self)
{
	return			(!!self->eof());
}

LPCSTR script_r_stringZ(IReader *self)
{
	shared_str		temp;
	self->r_stringZ	(temp);
	return			(*temp);
...
bool script_r_bool(IReader *self)
{
	return			(!!self->r_u8());
}

void script_r_fvector3(IReader *self, Fvector *arg0)
{
