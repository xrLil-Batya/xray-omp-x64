...
{
	return xr_new<CScriptIniFile>(update_custom_path(path, ini_file), false, false, false, false);
}

CScriptIniFile* read_ini(LPCSTR path, LPCSTR ini_file)
{
	return xr_new<CScriptIniFile>(update_custom_path(path, ini_file), true, true, false, false);
}
#pragma warning(pop)
