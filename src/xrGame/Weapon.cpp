...
void CWeapon::UpdateHudAdditonal(Fmatrix& trans)
{
	...
	if ((IsZoomed() && m_zoom_params.m_fZoomRotationFactor <= 1.f) || (!IsZoomed() && m_zoom_params.m_fZoomRotationFactor > 0.f))
	{
		...
		float src = 5 * Device.fTimeDelta * 0.65;
		float target = pActor->IsZoomAimingMode() ? 1.3f : -0.15f;
		clamp(src, 0.f, 1.f);
    ...
    if (HudInertionEnabled() && HudInertionAllowed())
    {
        ...
        // calc difference
        Fvector diff_dir;
		static Fvector inert_st_last_dir{};
        ...
        { // Худ в режиме "Прицеливание"
            ...
            _tendto_speed = hi->m_measures.m_inertion_params.m_tendto_ret_speed_aim - (hi->m_measures.m_inertion_params.m_tendto_ret_speed_aim - hi->m_measures.m_inertion_params.m_tendto_ret_speed) * factor;
            _origin_offset = hi->m_measures.m_inertion_params.m_origin_offset_aim - (hi->m_measures.m_inertion_params.m_origin_offset_aim - hi->m_measures.m_inertion_params.m_origin_offset) * factor;
        }
        else
        { // Худ в режиме "От бедра"
            _tendto_speed = hi->m_measures.m_inertion_params.m_tendto_ret_speed;
            _origin_offset = hi->m_measures.m_inertion_params.m_origin_offset;
        }
        ...
        origin.mad(xform.k, -pitch * hi->m_measures.m_inertion_params.m_pitch_offset_d);

        // Сдвиг в противоположную часть экрана
        origin.mad(xform.i, -pitch * hi->m_measures.m_inertion_params.m_pitch_offset_r);

        // Подьём\опускание
        clamp(pitch, hi->m_measures.m_inertion_params.m_pitch_low_limit, PI);
        origin.mad(xform.j, -pitch * hi->m_measures.m_inertion_params.m_pitch_offset_n);
    }
}
