...
void stalker_movement_manager_smart_cover::loophole_path					(smart_cover::cover const &cover, shared_str const &source_raw, shared_str const &target_raw, LoopholePath &path) const
{
	...
	using CBaseParameters = GraphEngineSpace::CBaseParameters;
	CBaseParameters parameters(static_cast<_dist_type>(-1), static_cast<_index_type>(-1), static_cast<_iteration_type>(-1));
	path.clear		();
	R_ASSERT2				(
