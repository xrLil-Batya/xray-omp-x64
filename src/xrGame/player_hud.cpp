...
u32 attachable_hud_item::anim_play(const shared_str& anm_name_b, BOOL bMixIn, const CMotionDef*& md, u8& rnd_idx, bool wpn_mix)
{
	...
	if (!anm)
		anm = m_hand_motions.find_motion(anm_name_b);
...
{
	if(!m_attached_items[0] && !m_attached_items[1] && script_anim_part == u8(-1))	return;
...
u32 player_hud::script_anim_play(u8 hand, LPCSTR section, LPCSTR anm_name, bool bMixIn, float speed)
{
	...
	else if (hand == 2) // both hands
	{
		...
		R_ASSERT2(0, "An error has occured.");
	...
	Msg("[%s] Playing motion with length [%i]", __FUNCTION__, length);
	return length;
}
...
#pragma todo("xrLil Batya: временно отключено")
	if (true || !pActor)
		return;
