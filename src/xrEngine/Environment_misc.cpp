...
void CEnvDescriptor::load(CEnvironment& environment, CInifile& config)
{
	...
	auto m_sun_pos_config = xr_new<CInifile>(FS.update_path(file_name, "$game_config$", "environment\\sun_positions.ltx"), true, true, false);
    char sun_identifier[10];
    sprintf(sun_identifier, tm.x >= 10 ? "%d:00:00" : "0%d:00:00", tm.x);

	if(m_sun_pos_config->section_exist(sun_identifier))
	{
...
bool ENGINE_API IsImGuiWeatherEditorActive{};
