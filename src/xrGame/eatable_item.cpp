...
void CEatableItem::UpdateInRuck() {}
...
void CEatableItem::HideWeapon()
{
	...
	if (!Actor()->inventory_disabled() && CurrentGameUI() && &CurrentGameUI()->GetActorMenu())
		CurrentGameUI()->HideActorMenu();
}

void CEatableItem::OnEvent(NET_Packet& P, u16 type)
{
	if (type == GEG_PLAYER_START_ANIMATION)
	{
		m_bItmStartAnim = true;
		return;
	}
...
void CEatableItem::UpdateUseAnim()
{
	...
	CEffectorCam* effector{};
	if(auto actor = Actor(); actor && &actor->Cameras())
		effector = actor->Cameras().GetCamEffector((ECamEffectorType)effUseItem);
