...
void CCC_CreateGameSpyAccount::Execute(LPCSTR args)
{
	...
	sscanf_s(args, "%s %s %s %s",
		tmp_nick, static_cast<u32>(sizeof(tmp_nick)),
		tmp_unick, static_cast<u32>(sizeof(tmp_unick)),
		tmp_email, static_cast<u32>(sizeof(tmp_email)),
		tmp_password, static_cast<u32>(sizeof(tmp_password)));
...
void CCC_GameSpySuggestUNicks::Execute(LPCSTR args)
{
	...
	VERIFY(MainMenu() && MainMenu()->GetGS());
	string256	tmp_unick;
	sscanf_s(args, "%s", tmp_unick, static_cast<u32>(sizeof(tmp_unick)));
...
void CCC_GameSpyProfile::Execute(LPCSTR args)
{
	...
	sscanf_s(args, "%s", tmp_command, static_cast<u32>(sizeof(tmp_command)));
