...
xrGameSpyServer::EConnect xrGameSpyServer::Connect(shared_str &session_name, GameDescriptionData & game_descr)
{
	...
	const bool use_master_server = strstr(Core.Params, "public=1");
	m_iReportToMasterServer = use_master_server;//game->get_option_i		(*session_name,"public",0);
	Msg(m_iReportToMasterServer ? "m_iReportToMasterServer = true" : "m_iReportToMasterServer = false");
	m_iMaxPlayers	= game->get_option_i		(*session_name,"maxplayers",32);
//	m_bCheckCDKey = game->get_option_i		(*session_name,"cdkey",0) != 0;
	m_bCheckCDKey = use_master_server;
	//--------------------------------------------//
