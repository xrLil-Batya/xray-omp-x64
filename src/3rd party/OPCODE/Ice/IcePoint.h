...
// Removed since it introduced the nasty "Point T = *IceMatrix4x4.GetTrans();" bug.......
		...
		inline_	void			TweakBigger()
								{
									uqword	Dummy = (IR(x)&TWEAKNOTMASK);	if(x >= 0.f)	Dummy+=TWEAKMASK+1;	x = FR(Dummy);
											Dummy = (IR(y)&TWEAKNOTMASK);	if(y >= 0.f)	Dummy+=TWEAKMASK+1;	y = FR(Dummy);
											Dummy = (IR(z)&TWEAKNOTMASK);	if(z >= 0.f)	Dummy+=TWEAKMASK+1;	z = FR(Dummy);
								}
