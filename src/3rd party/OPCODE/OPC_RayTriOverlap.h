...
//		if(IR(u)&0x80000000 || u>det)					return FALSE;
		if(mStabbedFace.mU < 0.f || mStabbedFace.mU > det)		return FALSE;
		...
		if(mStabbedFace.mV < 0.f || mStabbedFace.mU+mStabbedFace.mV>det)	return FALSE;
		...
		if(mStabbedFace.mDistance < 0.f)								return FALSE;
		// Else go on
...
//		if(IR(u)&0x80000000 || u>1.0f)					return FALSE;
		if(mStabbedFace.mU < 0.f || IR(mStabbedFace.mU)>IEEE_1_0)		return FALSE;
		...
		if(mStabbedFace.mV < 0.f || mStabbedFace.mU+mStabbedFace.mV>1.0f)	return FALSE;
