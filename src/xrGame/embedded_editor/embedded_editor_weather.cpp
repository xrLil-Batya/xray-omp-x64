...
void ShowWeatherEditor(bool& show)
{
    ...
    if (ImGui::SliderFloat("Time factor", &tf, 0.0f, 1000.0f, "%.3f", 2))
    ...
    for (int i = 0; i != env.CurrentWeather->size(); i++)
        if (env.CurrentWeather->at(i) && cur->m_identifier == env.CurrentWeather->at(i)->m_identifier)
            sel = i;
	...
	for (int i = 0; i != env.m_suns_config->sections().size(); i++)
		if (env.m_suns_config->sections()[i] && cur->lens_flare_id == env.m_suns_config->sections()[i]->Name)
			sel = i;
	...
	for (int i = 0; i != env.m_thunderbolt_collections_config->sections().size(); i++)
		if (env.m_thunderbolt_collections_config->sections()[i] && cur->tb_id == env.m_thunderbolt_collections_config->sections()[i]->Name)
			sel = i + 1;
